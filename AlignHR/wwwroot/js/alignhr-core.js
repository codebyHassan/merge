...
AlignHR.Loading = {
  ...
  init() {
    ...
    // Show bar on navigation; reset lock so bfcache never restores a stuck busy state
  ...
  acquire(btn) {
    ...
    // Safety auto-release after 15s so a stuck lock never permanently blocks the UI
    clearTimeout(this._autoReleaseTimer);
    this._autoReleaseTimer = setTimeout(() => this.release(btn), 15000);
    return true;
  },
