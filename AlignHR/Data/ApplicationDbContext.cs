...
namespace AlignHR.Data
{
    public class ApplicationDbContext : DbContext
    {
       ...
       : base(options)
        ...
        public DbSet<HrPostingChannel> HrPostingChannels { get; set; }
        public DbSet<HrJobPostingChannel> HrJobPostingChannels { get; set; }
        ...
        public DbSet<HrOfferResponse> HrOfferResponses { get; set; }
        ...
        public DbSet<HrOnboardingDocument> HrOnboardingDocuments { get; set; }
        public DbSet<HrJoiningConfirmation> HrJoiningConfirmations { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<HrPostingChannel>()
                .ToTable("HR_PostingChannel");
            ...
            modelBuilder.Entity<HrJobPostingChannel>()
                .ToTable("HR_JobPostingChannel");

            modelBuilder.Entity<HrJobPostingChannel>()
                .HasOne(jpc => jpc.JobPosting)
                .WithMany(jp => jp.PostingChannels)
                .HasForeignKey(jpc => jpc.JobPostingFK)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<HrJobPostingChannel>()
                .HasOne(jpc => jpc.PostingChannel)
                .WithMany(pc => pc.JobPostingChannels)
                .HasForeignKey(jpc => jpc.PostingChannelFK)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<HrJobPostingChannel>()
                .HasIndex(jpc => new { jpc.JobPostingFK, jpc.PostingChannelFK })
                .IsUnique()
                .HasDatabaseName("IX_HR_JobPostingChannel_Posting_Channel_Unique");
            ...
            modelBuilder.Entity<HrOfferResponse>()
                .ToTable("HR_OfferResponse");

            modelBuilder.Entity<HrOfferResponse>()
                .HasOne(r => r.Offer)
                .WithMany(o => o.Responses)
                .HasForeignKey(r => r.OfferFk)
                .OnDelete(DeleteBehavior.Restrict);
            ...
            modelBuilder.Entity<HrOnboardingDocument>()
                .ToTable("HR_OnboardingDocument");

            modelBuilder.Entity<HrOnboardingDocument>()
                .HasOne(d => d.Onboarding)
                .WithMany(o => o.Documents)
                .HasForeignKey(d => d.OnboardingFk)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<HrJoiningConfirmation>()
                .ToTable("HR_JoiningConfirmation");

            modelBuilder.Entity<HrJoiningConfirmation>()
                .HasOne(j => j.Onboarding)
                .WithOne(o => o.JoiningConfirmation)
                .HasForeignKey<HrJoiningConfirmation>(j => j.OnboardingId)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<HrJoiningConfirmation>()
                .HasIndex(j => j.OnboardingId)
                .IsUnique()
                .HasDatabaseName("IX_HR_JoiningConfirmation_Onboarding_Unique");
            ...
            modelBuilder.Entity<DmsDefinationFile>()
                .HasOne(d => d.ApprovalTemplate)
                .WithMany()
                .HasForeignKey(d => d.ApprovalTemplateId)
                .OnDelete(DeleteBehavior.Restrict);
...
// ===== Approval System =====
             ...
             // ApprovalTemplateStep → ApprovalTemplate (cascade delete steps with template)
             ...
             // Unique index: one step number per template
             modelBuilder.Entity<ApprovalTemplateStep>()
                 .HasIndex(s => new { s.TemplateId, s.StepNo })
                 .IsUnique()
                 .HasDatabaseName("IX_ApprovalTemplateStep_Template_StepNo");

             // WorkflowCondition → ApprovalTemplate (cascade delete conditions with template)
             modelBuilder.Entity<WorkflowCondition>()
                 .HasOne(c => c.Workflow)
                 .WithMany(w => w.Conditions)
                 .HasForeignKey(c => c.WorkflowId)
                 .OnDelete(DeleteBehavior.Cascade);

             // WorkflowCondition → ThenWorkflow (optional conditional routing)
             modelBuilder.Entity<WorkflowCondition>()
                 .HasOne(c => c.ThenWorkflow)
                 .WithMany()
                 .HasForeignKey(c => c.ThenWorkflowId)
                 .OnDelete(DeleteBehavior.Restrict);

             // DocApprovalAssignment → User
             modelBuilder.Entity<DocApprovalAssignment>()
                 .HasOne(a => a.User)
                 .WithMany()
                 .HasForeignKey(a => a.UserId)
                 .OnDelete(DeleteBehavior.Restrict);

             // DocApprovalAssignment → ApprovalTemplate
             modelBuilder.Entity<DocApprovalAssignment>()
                 .HasOne(a => a.ApprovalTemplate)
                 .WithMany()
                 .HasForeignKey(a => a.ApprovalTemplateId)
                 .OnDelete(DeleteBehavior.Restrict);

             // DocApprovalInstance → Document
             modelBuilder.Entity<DocApprovalInstance>()
                 .HasOne(i => i.Document)
                 .WithMany()
                 .HasForeignKey(i => i.DocumentID)
                 .OnDelete(DeleteBehavior.Restrict);

             // DocApprovalInstance → ApprovalTemplate (workflow)
             modelBuilder.Entity<DocApprovalInstance>()
                 .HasOne(i => i.Workflow)
                 .WithMany()
             ...
             // DocApprovalInstance → User (who submitted)
             modelBuilder.Entity<DocApprovalInstance>()
             ...
             // Allow multiple instances per document (re-submission after rejection)
             modelBuilder.Entity<DocApprovalInstance>()
                 .HasIndex(i => new { i.DocumentID, i.Status })
                 .HasDatabaseName("IX_DocApprovalInstance_Document_Status");

             // DocApprovalInstanceStep → DocApprovalInstance (cascade delete steps with instance)
             modelBuilder.Entity<DocApprovalInstanceStep>()
                 .HasOne(s => s.Instance)
                 .WithMany(i => i.InstanceSteps)
                 .HasForeignKey(s => s.InstanceId)
                 .OnDelete(DeleteBehavior.Cascade);

             // DocApprovalInstanceStep → ApprovalTemplateStep
             modelBuilder.Entity<DocApprovalInstanceStep>()
             ...
             // DocApprovalInstanceStep → User (approver)
             modelBuilder.Entity<DocApprovalInstanceStep>()
                 .HasOne(s => s.Approver)
                 .WithMany()
             ...
             // DocApprovalInstanceStep → Department (for Department flow context)
             modelBuilder.Entity<DocApprovalInstanceStep>()
