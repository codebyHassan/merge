    ...
    public class HrRequisitionsController : BaseController
    {
        ...
        public async Task<IActionResult> Create(HrRequisitionFormViewModel model, string command)
        {
            ...
            LoadDropdowns(model.DepartmentFK, model.RequisitionType, model.Nature,
                model.ReplacementEmployeeId, model.TransferFromDepartmentId, model.TransferEmployeeId);
            return View(model);
        }
        ...
        public async Task<IActionResult> RecruiterInbox()
        {
            ...
            var assignedIds = assignments.Select(a => a.RequisitionFk!.Value).ToHashSet();

            var recruiterEmpIds = assignments
                .Where(a => a.RecruiterEmployeeFK.HasValue)
                .Select(a => (int)a.RecruiterEmployeeFK!.Value)
                .Distinct()
                .ToList();
            var recruiterNames = await _context.emp
                .Where(e => recruiterEmpIds.Contains(e.Id))
                .ToDictionaryAsync(e => e.Id, e => e.FullName);

            var assignmentByReq = assignments
                .Where(a => a.RequisitionFk.HasValue && a.RecruiterEmployeeFK.HasValue)
                .ToDictionary(a => a.RequisitionFk!.Value, a => (int)a.RecruiterEmployeeFK!.Value);
            ...
            var items = requisitions.Select(r => new HrRecruiterInboxItemViewModel
            {
                ...
                AssignedRecruiterName = assignmentByReq.TryGetValue(r.Id, out var empId) && recruiterNames.TryGetValue(empId, out var rn) ? rn : null
            }).ToList();
        ...
        public async Task<IActionResult> Details(decimal id)
        {
            ...
            ViewBag.TransferEmployeeName = model.TransferEmployeeId.HasValue
                ? _context.emp.Where(e => e.Id == (int)model.TransferEmployeeId.Value)
                    .Select(e => e.Code + " - " + e.FirstName + " " + e.LastName)
                    .FirstOrDefault()
                : null;
        ...
        public JsonResult GetEmployeesByDepartment(int departmentId)
        {
            var employees = _context.emp
                .Where(e => e.DepartmentFk == departmentId)
                .OrderBy(e => e.FirstName)
                .Select(e => new { id = e.Id, name = e.Code + " - " + e.FirstName + " " + e.LastName })
                .ToList();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ConfirmTransfer(decimal requisitionId, DateOnly effectiveDate)
        {
            var req = await _context.HrRequisitions.FirstOrDefaultAsync(r => r.Id == requisitionId);
            if (req == null) { TempData["Error"] = "Requisition not found."; return RedirectToAction(nameof(Index)); }

            if (!req.TransferEmployeeId.HasValue)
            {
                TempData["Error"] = "No employee linked to this transfer requisition.";
                return RedirectToAction(nameof(Details), new { id = requisitionId });
            }

            var employee = await _context.emp.FirstOrDefaultAsync(e => e.Id == (int)req.TransferEmployeeId.Value);
            if (employee == null) { TempData["Error"] = "Employee not found."; return RedirectToAction(nameof(Details), new { id = requisitionId }); }

            if (!req.DepartmentFK.HasValue)
            {
                TempData["Error"] = "Target department not set on this requisition.";
                return RedirectToAction(nameof(Details), new { id = requisitionId });
            }

            var fromDeptId = employee.DepartmentFk;
            employee.DepartmentFk = (int)req.DepartmentFK.Value;
            ...
            await _context.EmployeeHireHistories.AddAsync(new EmployeeHireHistory
            {
                ...
                HireType = "Transfer",
                RequisitionFk = req.Id,
                FromDepartmentId = fromDeptId,
                EffectiveDate = effectiveDate,
                CreatedBy = PermissionHelper.GetCurrentUsername(HttpContext),
                CreatedOn = DateTime.Now,
                Remarks = $"Transferred from dept {fromDeptId} via requisition {req.RequisitionNo}"
            });
            ...
            TempData["Success"] = $"Transfer confirmed. {employee.FirstName} {employee.LastName} moved to new department.";
            return RedirectToAction(nameof(Details), new { id = requisitionId });
        }
        ...
        private void LoadDropdowns(decimal? selectedDept = null,
            ...
            decimal? selectedTransferEmp = null)
        {
